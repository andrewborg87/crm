...
@Injectable()
@ValidatorConstraint({ name: 'tagId', async: true })
export class TagIdValidator implements ValidatorConstraintInterface, PipeTransform {
  constructor(
  ...
  async #exec(value: string): Promise<boolean> {
    if (!isUUID(value)) {
      throw new BadRequestException('tagId must be a valid uuid');
    }

    const entity = await this.repo.findOne({ where: { id: value } });
    if (!entity) {
      throw new BadRequestException(`tagId must reference an existing entity, ${value} provided.`);
    }
